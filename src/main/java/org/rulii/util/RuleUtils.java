...
public final class RuleUtils {
    ...
    /**
     * Generates a signature for the given data using parameter matches and values.
     *
     * @param data the data object to generate the signature for
     * @param matches the list of parameter matches associated with the data
     * @param values the list of values corresponding to the parameter matches
    ...
    /**
     * Converts a list of ParameterMatch objects to an unmodifiable map with values from the corresponding list.
     *
     * @param matches The list of ParameterMatch objects representing parameter definitions.
     * @param values The list of values corresponding to the parameter matches.
     * @return An unmodifiable map where the keys are parameter names and values are corresponding values.
     * @throws UnrulyException if the size of the*/
    ...
    /**
     * Retrieves the default rule name based on the provided rule class. The default rule name is generated by converting the first character of the rule class's simple name to lowercase
     *.
     *
     * @param ruleClass the rule class for which to retrieve the default rule name
     * @return the default rule name based on the rule class
     */
    public static String getDefaultRuleName(Class<?> ruleClass) {
        Assert.notNull(ruleClass, "ruleClass cannot be null.");
        return Character.toLowerCase(ruleClass.getSimpleName().charAt(0)) + ruleClass.getSimpleName().substring(1);
    }
