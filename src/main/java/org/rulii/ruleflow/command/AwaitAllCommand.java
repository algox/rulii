...
/**
 * Pipeline command that blocks until <em>all</em> named {@link CompletableFuture} bindings
 * have completed (successfully or exceptionally).
 *
 * <p>Individual futures are not replaced after completion — they remain as
 * {@code CompletableFuture<T>} bindings. If any future completes exceptionally the combined
 * wait still runs, but the first failure is re-thrown as an {@link UnrulyException}.
 *
 ...
 * @since 2.0
 */
public class AwaitAllCommand implements RuleFlowCommand {

    private final long timeout;
    private final TimeUnit timeUnit;
    private final String[] bindingNames;

    public AwaitAllCommand(long timeout, TimeUnit timeUnit, String... bindingNames) {
        super();
    ...
    public void execute(RuleFlowExecutionContext ctx) {
        ...
        try {
            ...
            throw new UnrulyException("awaitAll " + Arrays.toString(bindingNames)
                    + " timed out after " + timeout + " " + timeUnit + ".", e);
        } catch (ExecutionException e) {
            Throwable cause = e.getCause() != null ? e.getCause() : e;
            throw cause instanceof UnrulyException ue
                    ? ue
                    : new UnrulyException("One or more async steps failed: " + Arrays.toString(bindingNames) + ".", cause);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new UnrulyException("awaitAll " + Arrays.toString(bindingNames) + " interrupted.", e);
        }
    }
