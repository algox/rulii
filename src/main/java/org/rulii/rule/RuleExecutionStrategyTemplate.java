...
public abstract class RuleExecutionStrategyTemplate implements RuleExecutionStrategy {
     ...
     */
    protected NamedScope createRuleScope(Rule rule, RuleContext ruleContext) {
        Assert.notNull(rule, "rule cannot be null.");
        Assert.notNull(ruleContext, "ruleContext cannot be null.");
        NamedScope result = ruleContext.getBindings().addScope(getRuleScopeName(rule));
        ...
        bindings.promiscuousBind(Binding.builder()
                .with(ReservedBindings.THIS.getName())
                .type(Rule.class)
                .value(rule)
                .build());
     ...
     */
    protected String getRuleScopeName(Rule rule) {
        return rule.getName() + "-scope-" + UUID.randomUUID();
    }
